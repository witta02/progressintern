      ...
      >
        ...
        <div class="p-6 border-b border-slate-100 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <span
              *ngIf="unreadCount > 0"
              class="px-2.5 py-0.5 bg-rose-500 text-white text-[10px] font-black rounded-full shadow-sm animate-pulse"
            >
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div
            *ngIf="hasNotificationSupport && permissionStatus !== 'granted'"
            class="p-5 rounded-2xl bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-100/80 shadow-sm flex flex-col gap-3 font-sans"
          >
            <div class="flex gap-3">
              <span class="text-xl">🔔</span>
              <div>
                <h5 class="text-sm font-black text-slate-800 leading-snug">
                  เปิดแจ้งเตือนบนอุปกรณ์
                </h5>
                <p class="text-xs text-slate-500 font-bold leading-normal mt-1">
                  รับทราบทันทีเมื่อพี่เลี้ยงมอบหมายงานใหม่ หรืออนุมัติคำขอ
                </p>
              </div>
            </div>
            <button
          ...
          >
            <div
              class="w-20 h-20 bg-slate-50 rounded-[2rem] flex items-center justify-center text-slate-300 mb-4 border border-slate-100/50"
            >
              <svg
                class="w-10 h-10"
                fill="none"
          ...
          >
            ...
            <span
              *ngIf="!notification.read"
              class="absolute top-4 right-10 w-2 h-2 bg-blue-600 rounded-full animate-pulse"
            ></span>
            ...
            <div
              ...
              [class.text-emerald-650]="notification.type === 'success'"
              [class.border-emerald-100]="notification.type === 'success'"
              [class.bg-rose-50]="notification.type === 'error'"
              [class.text-rose-650]="notification.type === 'error'"
              [class.border-rose-100]="notification.type === 'error'"
              [class.bg-amber-50]="notification.type === 'warning'"
              [class.text-amber-650]="notification.type === 'warning'"
              [class.border-amber-100]="notification.type === 'warning'"
              [class.bg-blue-50]="notification.type === 'info'"
              [class.text-blue-605]="notification.type === 'info'"
              [class.border-blue-100]="notification.type === 'info'"
            >
              <svg
            ...
            <div class="flex-1 pr-6 font-sans">
              <h5 class="text-sm font-black text-slate-900 mb-0.5 leading-snug">
                {{ notification.title }}
              </h5>
              ...
              <span
                class="text-[9px] font-black text-slate-400 uppercase tracking-wider block mt-2"
                >{{ getRelativeTime(notification.timestamp) }}</span
              >
            </div>
          </div>
        </div>
